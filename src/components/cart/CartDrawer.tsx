                ...
                items.map((item) => (
                  <div key={item.id} className="flex gap-5 items-start group">
                    <Link
                      ...
                      onClick={() => setIsOpen(false)}
                      className="relative w-28 h-28 rounded-xl overflow-hidden border border-black/5 bg-gray-50 shrink-0 shadow-sm cursor-pointer"
                    >
                      <Image
                        src={item.image}
                        alt={item.name}
                    ...
                    <div className="flex-1 flex flex-col justify-between h-28 py-1">
                      <div className="flex justify-between items-start gap-2">
                        <Link
                          href={`/product/${item.id}`}
                          onClick={() => setIsOpen(false)}
                          className="font-serif font-medium text-[#1A1A1A] leading-tight line-clamp-2 text-lg hover:text-neon-pink transition-colors cursor-pointer"
                        >
                          {item.name}
                        </Link>
